...
// Call parent function to show tooltip
  ...
  initChart(parent) {
    ...
    svg.attr('transform', 'translate(' + this.container.width / 2 + ',' + this.container.height / 2 + ')');
    ...
    d3.select(window)
      .on('load', parent.change(this.data, parent));
  }
  ...
  private drawSVG(parent) {
    ...
    const arcGroup: any = chart.selectAll('.arc')
      .data(pie(this.data))
      .enter().append('g')
      .attr('class', 'arc');

    const pieSlice = arcGroup.append('path')
      .style('fill', (d: any) => {
    ...
    // label
    let label = arcGroup.append('text')
      .attr('class', 'pieLabel')
  ...
  private deselectArea(parent) {
    ...
    $('#' + parent.attachNode + ' .legend .key').find('rect').add('text').attr('fill-opacity', '1.0');
