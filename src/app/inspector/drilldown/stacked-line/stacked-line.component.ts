...
// Remove empty data rows
  ...
  private initChart(parent): void {
    ...
    const y: any = d3.scaleLinear()
      .range([this.container.height, 5]);


    x.domain(this.data.map((d: any) => {
      return d.key as any;
    }));
  ...
  private drawSVG(parent) {
    ...
    const svg = d3.select('#' + this.attachNode)
      ...
      .attr('viewBox', '0, 0, ' + (this.container.width +
        this.margin.left + this.margin.right) + ', ' + (this.container.height + this.margin.top + this.margin.bottom));
    this.svg = svg;
    ...
    const hGrid = chart.selectAll('line.horizontalGrid')
